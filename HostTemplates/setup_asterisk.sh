...
# --- 1. FETCH MACHINESTATUS SOURCE FROM LOCAL PC ---
echo "--- Step 1: Copy MachineStatus source from your PC ---"
echo ""
read -p "  Enter your PC's IP address       : " HOST_IP
read -p "  Enter your PC's username         : " HOST_USER
read -p "  Enter full path to MachineStatus : " HOST_PATH
echo ""
...
chown -R pi:pi /home/pi/Documents/MachineStatus
echo "-> MachineStatus copied to /home/pi/Documents/MachineStatus"
...
exten => _XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX,1,Verbose(1,"User dialed ${EXTEN}")
 ...
 same => n,System(/home/pi/Documents/MachineStatus/dtmf.sh start_dialing ${PHONE} ${CALLERID_NUM} ${USER_ID} ${DIALPLAN} &)
...
exten => h,1,NoOp(Final Dial Status: ${DIALSTATUS})
 ...
 same => n,ExecIf($["${DIALSTATUS}" = "ANSWER"]?System(/home/pi/Documents/MachineStatus/dtmf.sh completed ${PHONE} ${CALLERID_NUM} ${USER_ID} ${DIALPLAN} &))
 ; If the call was NOT answered (BUSY, NOANSWER, CONGESTION, etc.), run the not_answered script
 same => n,ExecIf($["${DIALSTATUS}" != "ANSWER"]?System(/home/pi/Documents/MachineStatus/dtmf.sh not_answered ${PHONE} ${CALLERID_NUM} ${USER_ID} ${DIALPLAN} ${DIALSTATUS} &))
 same => n,Hangup()
...
echo "================================================="
echo "  SUCCESS!                                        "
echo "  - MachineStatus: copied to ~/Documents          "
