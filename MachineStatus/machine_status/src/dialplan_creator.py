...
def check_capmaign_exists(campaign):
    with open('/home/pi/Documents/MachineStatus/asterisk_dialplan.conf', 'r') as file:  # Path to your extensions.conf file
        for line in file:
...
def create_dialplan(all_data):
    ...
    try:
        with open('/home/pi/Documents/MachineStatus/asterisk_dialplan.conf', 'a') as file:  # Path to your extensions.conf file
...
exten => s,1,Verbose(1, "Call is answered ${{ARG1}}")
 ...
 same => n, SYSTEM(/home/pi/Documents/MachineStatus/dtmf.sh answered ${{PHONE}} ${{CALLERID_NUM}} ${{USER_ID}} ${{DIALPLAN}} &)
 ...
 same => n, GoTo({campaign}-{min_extension_id},s,1)
            ...
            for item in sorted_data:
                ...
                if item['sms_after'] is not None and item['template_id'] is not None:
                    send_sms_api = f"same => n, System(/home/pi/Documents/MachineStatus/dtmf.sh send_sms ${{PHONE}} ${{CALLERID_NUM}} ${{USER_ID}} ${{DIALPLAN}} {item['template_id']['id']} &)"
...
exten => {dtmf},1,Verbose(1, "Current Extension is ${{current_extension}}")
 same => n, System(/home/pi/Documents/MachineStatus/dtmf.sh dtmf ${{PHONE}} ${{CALLERID_NUM}} ${{USER_ID}} ${{DIALPLAN}} ${{current_extension}} ${{EXTEN}} &)"""
