...
def machine_status(request):
    # Extract host and password from query parameters
    ...
    # Check if provided host and password match the ones in settings
    if host != API_CREDENTIALS.get('HOST') or password != API_CREDENTIALS.get('SYSTEM_PASSWORD'):
        return JsonResponse({"error": "Invalid host or password."}, status=403)


    # Continue processing if credentials are valid
    machine_status = MachineStatus(ussd_forcefully, port_forcefully)
    ...
    data_list = machine_status.data_list
    final_data = {
        "host":host,
        "machine_power_status": machine_status.is_machine_on,
        ...
        "total_sms_balance": machine_status.total_sms_balance,
...
def get_machine_status():
        # Read host, password, and port from settings
        credentials = settings.API_CREDENTIALS
        host = credentials['HOST']
        password = credentials['SYSTEM_PASSWORD']
        port = settings.PORT  # Assuming you have PORT defined in your settings

        # Check if provided host, password, and port match the ones in settings
        if host and password and port:
            # Construct the URL
            url = f"http://localhost:{port}/machine_status?host={host}&password={password}"
            ...
            try:
                ...
                # If parsing fails, assume it's already a dictionary
                return response
...
def send_sms(request):
    ...
    data = data.get('data')  # Get list of combined data

    # Check if provided host and password match the ones in settings
    credentials = read_api_credentials()
    if host != credentials['host'] or password != credentials['system_password']:
        return JsonResponse({"error": "Invalid host or password."}, status=403)

    responses = []
    machine_status_response = get_machine_status()
    if not machine_status_response:
        return JsonResponse({"error": "Failed to get machine status"}, status=500)
    ...
    for item in data:
        ...
        port_data = next((pd for pd in machine_status_response.get('port_data', []) if pd.get('port') == port), None)
...
LOG_FILE_PATH = '/home/pi/Documents/MachineStatus/dtmf_call_maker.log'
...
def make_call(request):
    if request.method == 'POST':
        ...
        # Now verify host and system password
        if host != API_CREDENTIALS.get('HOST') or system_password != API_CREDENTIALS.get('SYSTEM_PASSWORD'):
            response_data = {'error': 'Invalid host or password'}
            logger.warning(f"RESPONSE 403: {response_data} | User: {user_id}")
...
def change_host_password(request):
    if request.method == 'POST':
        ...
        # Now verify host and system password
        if host != settings.API_CREDENTIALS.get('HOST') or system_password != settings.API_CREDENTIALS.get('SYSTEM_PASSWORD'):
            return JsonResponse({'error': 'Invalid host or password'}, status=403)

        # Update settings.py
        settings_file_path = os.path.join(settings.BASE_DIR, 'machine_status', 'settings.py')
        ...
        # Update dtmf.sh
        dtmf_file_path = '/home/pi/Documents/MachineStatus/dtmf.sh'
...
def save_dial_plan(request):
    ...
    if request.method == 'POST':
        ...
        system_password = all_data.get('system_password')
        campaign = all_data.get('campaign')
        print(f"[save_dial_plan] Host={host}, Campaign={campaign}")

        # Now verify host and system password
        if host != API_CREDENTIALS.get('HOST') or system_password != API_CREDENTIALS.get('SYSTEM_PASSWORD'):
            print("[save_dial_plan] Authentication failed")
            return JsonResponse({'error': 'Invalid host or password'}, status=403)
        print("[save_dial_plan] Authentication succeeded")
...
def reboot(request):
    if request.method == 'POST':
        ...
        try:
            script_path = '/home/pi/Documents/MachineStatus/machine_status/src/reboot.sh'
...
def update_code(request):
    if request.method == 'POST':
        # Get the host and system password from the URL
        host = request.GET.get('host')
        system_password = request.GET.get('system_password')

        # Check if the host and system password match the ones in settings.py
        if host != settings.API_CREDENTIALS['HOST'] or system_password != settings.API_CREDENTIALS['SYSTEM_PASSWORD']:
            return HttpResponseForbidden('<h1>Invalid host or system password</h1>')
        print("Host and Password verified")
        ...
        temp_dir_machine_status = '/home/pi/Documents/temp/MachineStatus'
        ...
        if response.status_code == 200:
            file_name = os.path.join(temp_dir, 'MachineStatus.zip')
        ...
        #     file_name = os.path.join(temp_dir, 'MachineStatus.zip')
            ...
            try:
                with zipfile.ZipFile(file_name, 'r') as zip_ref:
                    zip_ref.extractall(temp_dir_machine_status)
                print("Zip file extracted successfully")

                # Change password and host in settings.py
                settings_file_path = os.path.join(temp_dir_machine_status, 'machine_status/machine_status/settings.py')
                with open(settings_file_path, 'r') as file:
                    lines = file.readlines()
                ...
                script_path = '/home/pi/Documents/MachineStatus/update.sh'
                subprocess.run(['sh', script_path])
...
def zip_entire_code(request):
    ...
    if host != settings.API_CREDENTIALS['HOST'] or system_password != settings.API_CREDENTIALS['SYSTEM_PASSWORD']:
        return HttpResponseForbidden('<h1>Invalid host or system password</h1>')
    print("host and system password verified")

    # Define the source and destination paths
    machine_status_path = '/home/pi/Documents/MachineStatus'
    temp_path_root = '/home/pi/Documents/temp'  
    temp_path = '/home/pi/Documents/temp/MachineStatus'  # Update the path as needed

    # Create the temp directory if it does not exist
    os.makedirs(os.path.dirname(temp_path), exist_ok=True)
    print("temp directory created")

    zip_file_path = '/home/pi/Documents/MachineStatus.zip'  # Update the path
    # Copy the entire directory to a new temporary host
    shutil.copytree(machine_status_path, temp_path, symlinks=True)

    print("Code copied successfully")
    # Update host and system password in settings.py to empty strings
    settings_file_path = os.path.join(temp_path, 'machine_status/machine_status/settings.py')
    ...
    url = request.build_absolute_uri('/Documents/MachineStatus.zip')
...
def download_zip(request):
    file_path = '/home/pi/Documents/MachineStatus.zip'
