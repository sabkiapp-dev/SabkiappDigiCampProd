...
def sms_dialers_list_to_send(host, system_password, sms_dialers_list, gateway_status):
    ...
    port_data = sorted(gateway_status['port_data'], key=lambda x: x['sms_balance'], reverse=True)
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for host in booked_hosts:
                ...
                if user_hosts:
                    gateway_status = get_gateway_status(user_hosts.first())
                    if gateway_status and gateway_status['machine_power_status'] == 1 and gateway_status['total_sms_balance'] > 0:
                        # Find the maximum balance of the user hosts from the machine status
                        max_balance = 0
                        for port in gateway_status['port_data']:
                            if port['sms_balance'] > max_balance:
                                max_balance = port['sms_balance']

                        

                        limit = gateway_status['number_of_sims_ready'] + gateway_status['number_of_sims_busy']
                        if limit > 0:
                            sms_dialers_list = []
                            ...
                            if (len(sms_dialers_list) > 0):  
                                ...
                                response = sms_dialers_list_to_send(host, system_password, sms_dialers_list, gateway_status)
                                if(response):
