import time
import logging
# logging.basicConfig(filename='gateway_status.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def merge_sim_information(gateway_status):
    for status in gateway_status:
...
def fetch_gateway_status(user_id, host_forcefully, port_forcefully, ussd_forcefully, sms=False):
    ...
    for user_host in user_hosts:
        ...
        try:
            if ussd_forcefully == "1" and host_forcefully == host:
                ...
                response = requests.get(f'https://{host}.sabkiapp.com/gateway_status', params={'host': host, 'password': system_password, 'port_forcefully': port_forcefully, 'ussd_forcefully': ussd_forcefully}, timeout=10)
            else:    
                response = requests.get(f'https://{host}.sabkiapp.com/gateway_status', params={'host': host, 'password': system_password}, timeout=10)
...
def get_all_gateway_status(request):
    ...
    if auth_header:
        ...
        try:
            ...
            responses = fetch_gateway_status(user_id, host, port_forcefully, ussd_forcefully, sms)
