...
pickle_file_path_ms = 'machine_status_fetch_time.pkl'
...
def make_call_thread(final_dial_list: list[dict]) -> None:
    ...
    for dial in final_dial_list:
        ...
        if dial["name_spell"] != 0 and name is None:
            from voiceapi.api.models import get_name  # local import avoids cycle
...
def get_all_active_hosts():
    ...
    # TODO: Re-enable time/date filters before production
            ...
            if not call_status:
                ...
                if call_through is not None and user_id is not None:
                    ...
                    if sim_info:
                        ...
                        if user_host:
                            machine_status = get_machine_status(user_host)
                            if machine_status:
                                port = None
                                for item in machine_status['port_data']:
